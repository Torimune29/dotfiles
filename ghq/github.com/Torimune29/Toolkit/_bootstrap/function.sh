...
if ! $GATHERED_FACTS ; then
  if [ -x "$(command -v apt-get)" ];    then TOOLKIT_PACKAGE_MANAGER="apt";    TOOLKIT_PACKAGE_INSTALL="apt-get install -y -qq";     TOOLKIT_PACKAGE_UPDATE="apt-get update"
...
install_package () {
  ...
  set +e; eval $_escalation $TOOLKIT_PACKAGE_INSTALL $_package; set -e
